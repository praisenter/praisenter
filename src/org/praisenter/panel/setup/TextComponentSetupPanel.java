 ...
 * @since 1.0.0
 */
public class TextComponentSetupPanel extends JPanel implements ItemListener, ChangeListener, ActionListener {
	/** The version id */
	 ...
	 */
	public TextComponentSetupPanel(TextComponent component) {
		...
		if (font == null) {
			font = TextSettings.DEFALUT_FONT;
		}
	...
	public void itemStateChanged(ItemEvent event) {
		...
		if (source == this.cmbFontFamilies) {
			...
			if (old == null) {
				old = TextSettings.DEFALUT_FONT;
			}
			Font font = new Font(family, old.getStyle(), old.getSize());
			this.component.setTextFont(font);
			this.firePropertyChange(DisplaySetupPanel.DISPLAY_COMPONENT_PROPERTY, old, font);
	...
	public void stateChanged(ChangeEvent event) {
		...
		if (source == this.chkWrapText) {
			...
			if (nv instanceof Number) {
				...
				this.firePropertyChange(DisplaySetupPanel.DISPLAY_COMPONENT_PROPERTY, old, padding);
			}
		}
	}
	...
	public void actionPerformed(ActionEvent event) {
		...
		if ("textColor".equals(command)) {
			...
			this.firePropertyChange(DisplaySetupPanel.DISPLAY_COMPONENT_PROPERTY, old, TextAlignment.LEFT);
		} else if ("align-center".equals(command)) {
			TextAlignment old = this.component.getTextAlignment();
			this.component.setTextAlignment(TextAlignment.CENTER);
			this.firePropertyChange(DisplaySetupPanel.DISPLAY_COMPONENT_PROPERTY, old, TextAlignment.CENTER);
		} else if ("align-right".equals(command)) {
			TextAlignment old = this.component.getTextAlignment();
			this.component.setTextAlignment(TextAlignment.RIGHT);
			this.firePropertyChange(DisplaySetupPanel.DISPLAY_COMPONENT_PROPERTY, old, TextAlignment.RIGHT);
		}
	}
