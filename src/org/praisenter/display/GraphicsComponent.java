...
public class GraphicsComponent extends DisplayComponent {
	...
	private boolean boundsIncreased;
	...
	public void render(Graphics2D graphics) {
		...
		int dw = this.width;
		int dh = this.height;
		// FIXME should be able to cache the background graphics separately for improved performance
		// FIXME the displays that im sending from the bible/notification panel are created from one screen but displayed on another
		GraphicsConfiguration gc = graphics.getDeviceConfiguration();
		
		// make sure the source image is compatible
		if (this.backgroundImageVisible && this.backgroundImage != null && !this.imageConverted) {
			...
			this.backgroundImage = gc.createCompatibleImage(original.getWidth(), original.getHeight(), original.getTransparency());
		...
		if (this.isDirty() || this.cachedImage == null) {
			...
			if (this.cachedImage == null || this.boundsIncreased) {
				// create a compatible image
				this.cachedImage =  gc.createCompatibleImage(this.width, this.height, Transparency.TRANSLUCENT);
			...
			ig.clearRect(0, 0, this.cachedImage.getWidth(), this.cachedImage.getHeight());
			...
			if (this.backgroundImageVisible && this.backgroundImage != null) {
				...
				if (this.scaledImage == null) {
					...
					if (this.backgroundImageScaleType == ScaleType.UNIFORM) {
						this.scaledImage = ImageUtilities.getUniformScaledImage(this.backgroundImage, dw, dh, this.backgroundImageScaleQuality.getQuality());
					} else if (this.backgroundImageScaleType == ScaleType.NONUNIFORM) {
						this.scaledImage = ImageUtilities.getNonUniformScaledImage(this.backgroundImage, dw, dh, this.backgroundImageScaleQuality.getQuality());
					} else {
						// scaling type was none, so dont scale
						this.scaledImage = this.backgroundImage;
					}
				}
				// center the image
	...
	public void resize(int dw, int dh) {
		...
		if (this.cachedImage != null) {
			if (this.width > this.cachedImage.getWidth() || this.height > this.cachedImage.getHeight()) {
				// re-allocate the buffer
				this.boundsIncreased = true;
			}
			// the scaled image must be updated whenever the bounds are resized
			this.scaledImage = null;
		}
		this.setDirty(true);
	}
