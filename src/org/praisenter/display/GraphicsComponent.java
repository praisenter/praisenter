...
public class GraphicsComponent extends DisplayComponent {
	...
	private BufferedImage cachedCompositeImage;
	...
	public void render(Graphics2D graphics) {
		...
		this.render(graphics, configuration);
	}
	
	/* (non-Javadoc)
	 * @see org.praisenter.display.DisplayComponent#render(java.awt.Graphics2D, java.awt.GraphicsConfiguration)
	 */
	@Override
	public void render(Graphics2D graphics, GraphicsConfiguration configuration) {
		// generate any graphics
	...
	/* (non-Javadoc)
	 * @see org.praisenter.display.DisplayComponent#prepare(java.awt.GraphicsConfiguration)
	 */
	@Override
	public void prepare(GraphicsConfiguration configuration) {
		// cache any images
		this.cacheImages(configuration, this.boundsChangeType);
		this.boundsChangeType = null;
	...
	protected void renderComponent(Graphics2D graphics) {
		...
		if (this.backgroundImageVisible) {
			// render a scaled version of the image
			if (this.backgroundImage != null) {
				...
				if (this.backgroundImageScaleType == ScaleType.UNIFORM) {
					scaled = ImageUtilities.getUniformScaledImage(this.backgroundImage, w, h, this.backgroundImageScaleQuality.getQuality());
				} else if (this.backgroundImageScaleType == ScaleType.NONUNIFORM) {
					scaled = ImageUtilities.getNonUniformScaledImage(this.backgroundImage, w, h, this.backgroundImageScaleQuality.getQuality());
				} else {
					// scaling type was none, so don't scale
					scaled = this.backgroundImage;
				}
		...
		if (this.backgroundColorVisible && this.backgroundColorCompositeType == CompositeType.OVERLAY) {
			// render the color to the image after the image
			graphics.setColor(this.backgroundColor);
			// fill the whole image
			graphics.fillRect(0, 0, w, h);
		}
