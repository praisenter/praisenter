...
final class MediaDeleteTagAction implements AsyncAction {
	...
	public Void call() {
		...
		this.context.getMediaLibrary().removeTag(
			...
			(Tag failedTag, Throwable error) -> {
				// log the error
				LOGGER.error("Failed to remove tag '{}' for '{}': {}", failedTag.getName(), this.media.getPath().toAbsolutePath().toString(), error.getMessage());
				// show an error to the user
				Alert alert = Alerts.exception(
						this.owner,
						null, 
						null, 
						MessageFormat.format(Translations.get("tags.remove.error"), failedTag.getName()), 
						error);
				alert.show();
